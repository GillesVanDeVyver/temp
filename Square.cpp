...
Square::Optional Square::fromName(const std::string& name){
    Coordinate file = name[0] - 97;
    Coordinate rank = name[1] - 49;
    if ((name.length() == 2) && (file<=7)&& (rank<=7)){
        Index new_index = rank * 8 + file;
        Square newSquare(new_index);
        return newSquare;
    }
    return std::nullopt;  
}
